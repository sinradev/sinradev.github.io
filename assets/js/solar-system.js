...
(function() {
    ...
    function drawSphere(x, y, radius, color, glowColor, scale) {
        ...
        const lightColor = color.replace('#', '#').length === 7 ?
            'rgba(' + parseInt(color.substr(1,2),16) + ',' + parseInt(color.substr(3,2),16) + ',' + parseInt(color.substr(5,2),16) + ', 0.9)' :
            color;
    ...
    function drawSun(x, y, scale) {
        ...
        // Outer halo
        const haloGradient = ctx.createRadialGradient(x, y, radius * 0.5, x, y, radius * 2.5);
